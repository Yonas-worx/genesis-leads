...
$w('#button6').onClick(() => {
    ...
    dataset1.getItems(0, dataset1.getTotalCount())
        .then((results) => {
            ...
            const fileName = `AfterSales-${currentCountry}-${yyyy}${mm}${dd}-${hh}${min}-leads.csv`;
...
$w("#clearFiltersBtn").onClick((event) => {
    ...
    filterDataset($w("#dataset1"), filterCountry, null, null, null, null, null, null).then((filteredRes) => {
        filterDatesArr = null;
        filterShowroom = null;
        filterVehicle = null;
        filterSource = null;
