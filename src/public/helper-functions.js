...
export function setupSummaryTable(results) {
    ...
    summaryTableElement.rows = [{
        ...
        "Instagram & Facebook": results.items.filter(item => item.source === "Social" || item.source === "IG" || item.source === "FB").length,
    ...
    for (let country of listOfCountries) {
        ...
        const totalInstaFace = results.items.filter(item => item.country === country && (item.source === "Social" || item.source === "IG" || item.source === "FB")).length;
